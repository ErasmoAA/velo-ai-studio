...
title_en: "Text-to-Speech in AI videos: add automatic voiceover"
desc_es: Aprende a usar la función de Text-to-Speech de Velo AI Studio para añadir una voz narrativa automática a tus videos sin grabar ni un segundo de audio.
desc_en: Learn how to use Velo AI Studio's Text-to-Speech feature to add automatic narration to your videos without recording a single second of audio.
...
content_es: |
  ...
  1. **Sube tu archivo SRT** — La función de TTS requiere que uses un archivo SRT como entrada, ya que el sistema necesita el texto escrito para generar la voz.
  2. **Activa "Generar Voz" antes de ejecutar el pipeline** — En la sección de configuración verás un interruptor llamado **"Generar Voz (TTS)"**. Actívalo antes de iniciar la generación.
  3. **El audio se sincroniza con las imágenes** — El video final tendrá la voz narrando el texto de cada segmento SRT mientras aparece la imagen correspondiente.
...
content_en: |
  ...
  1. **Upload your SRT file** — The TTS feature requires an SRT file as input, since the system needs the written text to generate the voice.
  2. **Enable "Generate Voice" before running the pipeline** — In the configuration section you'll see a toggle called **"Generate Voice (TTS)"**. Enable it before starting generation.
  3. **The audio syncs with the images** — The final video will have the voice narrating the text of each SRT segment while the corresponding image appears.
